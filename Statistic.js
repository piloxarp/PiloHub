...
const AnswerYUECComics = countNames(YUEC) - countElementsWithParentheses(YUEC);
const AnswerNudeMoonComics = Comics.reduce((total, item) => total + item.count, 0);
const AnswerMcomicsComics = ( countNames(Mcomics) - countElementsWithParentheses(Mcomics) ) - 1;
...
const AnswerYUECArts = YUEC.reduce((total, item) => total + item.count, 0);
...
document.querySelectorAll("#stat_YUEC > nav > h4")[0].innerHTML = "Comics: " + AnswerYUECComics;
document.querySelectorAll("#stat_YUEC > nav > h4")[1].innerHTML = "Solo arts: " + AnswerYUECArts;

document.querySelector("#stat_TotalComics > nav > h4").innerHTML = AnswerBBCPackComics + AnswerGelatomusumePackComics + AnswerPicturdPackComics + AnswerOtherComics + AnswerFiveTComics  + AnswerHyanmaruComics + AnswerNudeMoonComics;
document.querySelector("#stat_TotalPages > nav > h4").innerHTML = AnswerMcomicsPages;
document.querySelector("#stat_TotalSoloArts > nav > h4").innerHTML = AnswerGelatomusumePackArts + AnswerBBCPackArts + AnswerPicturdPackArts + AnswerOtherArts + AnswerHyanmaruArts + AnswerYUECArts + Answer5tArts;
